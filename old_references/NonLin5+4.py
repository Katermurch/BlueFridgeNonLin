...
if instrument_path not in sys.path:
    sys.path.append(instrument_path)

analyzer_path = (
    r"C:\Users\quantum1\Documents\Python\instr\python_interface\python_without_WX2184C"
)
...
wx_amps = [
    ...
    0.5,
    0.5,
]  # q1 ro wx.975,q2 sweep .1#[1.0,.975,.5,.5]##[1.0,0.85,1.5,.5]# wx_amps =[1,0.85,1.9,.5]#wx_amps =[.6,1.2,.5,.5]
...
if (
    run_rabi_ef
):  # rabi_ef(num_steps=51,sweep_time=200,pi_ge = 20,ssm_ge = -0.150,ssm_ef=-0.300,ef_amp=1,ROIF1= 0,ROI2= 0,q=0)
    num_steps = 101
    sweep_time = 200  # ns
    ef_amp = 1
    if which_qubit == 0:
...
if pis_nopi:  # run to correct two qubit rabi tomography
    ...
    if which_qubit == 0:
        ...
        # |g>, keeps population in ground state
...
if run_ramsey_quantum_effiency:
    ...
    ro_dur = 5000  # 2000
    if which_qubit == 0:
        Fs.ramsey_quantum_effiency(
            num_steps, ssm_ge, pi_ge_time, ROIF2, q=which_qubit, RO_ram_amp=RO_ram_amp
        )
    if which_qubit == 1:
        Fs.ramsey_quantum_effiency(
            num_steps, ssm_ge, pi_ge_time, ROIF1, q=which_qubit, RO_ram_amp=RO_ram_amp
        )
...
if parametric_coupling_time_domain:
    ...
    # if which_qubit == 0:
    #        Fs.rabi_ge(num_steps,sweep_time,ssm_ge,ROIF2,which_qubit)
    Fs.parametric_coupling_time_domain(
        num_steps,
        ssm_ge,
        pi_ge=pi_ge_time,
        ssm_para=f_parametric,
        spec_amp=0.5,
        ROIF1=ROIF1,
        ROIF2=ROIF2,
        q=which_qubit,
...
) = daq_programs_homo.run_daq_het_2q(
    ROIF1,
    ROIF2,
    deg_1=IQ_angle_q1,
    deg_2=IQ_angle_q2,
...
def make_hist(IQdata, ax_hist, labelstr):
    ...
    counts_IQ = ax_hist.hist(
        IQdata, bins=bins_IQ, histtype="step", orientation="vertical", label=labelstr
    )[0]
...
if which_qubit == 0:
    ...
    print("I histogram")
    bins_Ig2, counts_Ig2, gauss_xaxis_Ig2 = make_hist(
        rec_readout_vs_pats_2[0, :, 0], axI2, "I2g"
    )
    bins_Ie2, counts_Ie2, gauss_xaxis_Ie2 = make_hist(
        rec_readout_vs_pats_2[0, :, 1], axI2, "I2e"
    )

    _2, mu_Ig2, std_Ig2 = analysis.fit_gaussian_no_plot(
        rec_readout_vs_pats_2[0, :, 0], gauss_xaxis_Ig2, counts_Ig2, axI2
    )

    _2, mu_Ie2, std_Ie2 = analysis.fit_gaussian_no_plot(
        rec_readout_vs_pats_2[0, :, 1], gauss_xaxis_Ie2, counts_Ie2, axI2
    )
    ...
    print("Q histogram")
    bins_Qg2, counts_Qg2, gauss_xaxis_Qg2 = make_hist(
        rec_readout_vs_pats_2[1, :, 0], axQ2, "Q2g"
    )
    bins_Qe2, counts_Qe2, gauss_xaxis_Qe2 = make_hist(
        rec_readout_vs_pats_2[1, :, 1], axQ2, "Q2e"
    )

    _2, mu_Qg2, std_Qg2 = analysis.fit_gaussian_no_plot(
        rec_readout_vs_pats_2[1, :, 0], gauss_xaxis_Qg2, counts_Qg2, axQ2
    )

    _2, mu_Qe2, std_Qe2 = analysis.fit_gaussian_no_plot(
        rec_readout_vs_pats_2[1, :, 1], gauss_xaxis_Qe2, counts_Qe2, axQ2
    )
...
if run_sweep:
    ...
    out_Q1, out_I1, out_Q2, out_I2 = chevron.sweep_keithley(
        address,
        current_start,
        current_stop,
        current_steps,
...
if run_sweep_AWG:
    ...
    out_Q1, out_I1, out_Q2, out_I2 = chevron.sweep_DC_AWG(
        address,
        voltage_start,
        voltage_stop,
        voltage_steps,
...
if run_qubit_bnc_sweep:
    ...
    start_freq = 4.600 + 0.02
    stop_freq = 4.600 - 0.02
    freq_steps = 21
    pow_setting = 13
    out_Q1, out_I1, out_Q2, out_I2 = chevron.sweep_bnc_qubit(
        start_freq,
        stop_freq,
        freq_steps,
        num_steps,
...
if run_T1 or run_T1_ef:
    ...
    if Qrange_abs > Irange_abs:
        T1_ge_fit_vals, error_vals, T1_fit_data, _ = analysis.fit_exp_decay(
            times, Q, guess_vals=[-Qrange, 0.5, Q[-1]]
        )
    else:
        T1_ge_fit_vals, error_vals, T1_fit_data, _ = analysis.fit_exp_decay(
            times, I, guess_vals=[-Irange, 0.5, I[-1]]
        )
...
if run_ramsey:
    ...
    Irange = abs(np.max(I) - np.min(I))
    if Qrange > Irange:
        T2_fit_vals, _, _, _ = analysis.fit_sine_decay(
            times,
            Q,
            guess_vals=[
                0.389417674062825,
...
if run_ramsey_quantum_effiency:
    ...
    Irange = abs(np.max(I) - np.min(I))
    if Qrange > Irange:
        fit_vals, _, _, _ = analysis.fit_sine(
            times, Q, guess_vals=[0.16, 100, -67.76128980276253, -263.4006117568909]
        )
    if Irange > Qrange:
        fit_vals, _, _, _ = analysis.fit_sine(
            times, I, guess_vals=[0.16, 100, -67.76128980276253, -263.4006117568909]
        )
...
if run_ro_freq_sweep:
    ...
    if which_qubit == 0:
        for i in range(len(ro_freq)):
            ...
            print("I histogram")
            bins_Ig, counts_Ig, gauss_xaxis_Ig = make_hist(
                rec_readout_vs_pats_2[0, :, 0], axI, "Ig"
            )
            bins_Ie, counts_Ie, gauss_xaxis_Ie = make_hist(
                rec_readout_vs_pats_2[0, :, 1], axI, "Ie"
            )

            _, mu_Ig, std_Ig = analysis.fit_gaussian_no_plot(
                rec_readout_vs_pats_2[0, :, 0], gauss_xaxis_Ig, counts_Ig, axI
            )

            _, mu_Ie, std_Ie = analysis.fit_gaussian_no_plot(
                rec_readout_vs_pats_2[0, :, 1], gauss_xaxis_Ie, counts_Ie, axI
            )
            ...
            print("Q histogram")
            bins_Qg, counts_Qg, gauss_xaxis_Qg = make_hist(
                rec_readout_vs_pats_2[1, :, 0], axQ, "Qg"
            )
            bins_Qe, counts_Qe, gauss_xaxis_Qe = make_hist(
                rec_readout_vs_pats_2[1, :, 1], axQ, "Qe"
            )

            _, mu_Qg, std_Qg = analysis.fit_gaussian_no_plot(
                rec_readout_vs_pats_2[1, :, 0], gauss_xaxis_Qg, counts_Qg, axQ
            )

            _, mu_Qe, std_Qe = analysis.fit_gaussian_no_plot(
                rec_readout_vs_pats_2[1, :, 1], gauss_xaxis_Qe, counts_Qe, axQ
            )
            ...
            noise = (
                np.abs(std_Ig) + np.abs(std_Qg) + np.abs(std_Ie) + np.abs(std_Qg)
            ) / 4
...
if run_ramsey_quantum_effiency_sweep:
    ...
    if which_qubit == 0:
        for i in range(len(RO_ram_ro_amp)):
            ...
            if Qrange > Irange:
                fit_vals, _, _, _ = analysis.fit_sine_fix_freq(
                    times,
                    Q,
                    guess_vals=[100, -67.76128980276253, 1500],
        ...
        Gaussian_fit_vals, _, _, _ = analysis.fit_gaussian_points(
            RO_ram_ro_amp, ramsey_amp, guess_vals=[300, 0, 1]
        )
...
if run_TWPA_snr_sweep:
    ...
    if which_qubit == 0:
        ...
        for j in range(len(twpa_freq)):
            for i in range(len(twpa_amp)):
                bnc.set_bnc_output(
                    twpa_freq[j], power_dBm=twpa_amp[i], bnc_addr=big_agilent_address
                )
                wx.wx_set_and_amplitude_and_offset(amp=wx_amps, offset=wx_offs)
                (
                    n_vs_pats_1,
                ...
                signal = np.sqrt((mu_Ig - mu_Ie) ** 2 + (mu_Qg - mu_Qe) ** 2)
                noise = (
                    np.abs(std_Ig) + np.abs(std_Qg) + np.abs(std_Ie) + np.abs(std_Qe)
                ) / 4
        ...
        plt.imshow(
            data_snr,
            aspect="auto",
            extent=[freq_start, freq_stop, amp_start, amp_stop],
            origin="lower",
        )
...
if run_TWPA_snr_sweep_corrected:
    ...
    try:
        for freq in twpa_freq:
            ...
            try:
                for amp in twpa_amp:
                    bnc.set_bnc_output(
                        freq, power_dBm=amp, bnc_addr=big_agilent_address
                    )
                    wx.wx_set_and_amplitude_and_offset(amp=wx_amps, offset=wx_offs)
                    (
                        n_vs_pats_1,
                ...
                data_df.to_csv(
                    snr_output_file, index=False
                )  # Save to file after each freq
    ...
    plt.imshow(
        data_snr, aspect="auto", extent=[7, 9, amp_start, amp_stop], origin="lower"
    )
    plt.colorbar(label="SNR")
    plt.xlabel("TWPA Frequency (GHz)")
    plt.ylabel("TWPA Amplitude (dBm)")
