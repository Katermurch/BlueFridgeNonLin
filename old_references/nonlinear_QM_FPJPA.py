...
def loading(num_steps=51, sweeptime=5000):
    ...
    ringupdown_seq.load_sequence_from_disk(
        "128.252.134.31", base_name="foo", file_path=write_dir, num_offset=0
    )
...
def test_single_generator(
    ...
    if True:
        ...
        plt.figure()
        plt.imshow(
            channel[:, file_length - 6000 - 300 : file_length - 6000 + 50],
            aspect="auto",
        )
        plt.show()
...
def freqsweep_rabi(freq=np.arange(0.1, 0.12, 0.001), singleave=5, ifsave=0):
    ...
    for k in np.arange(np.size(freq)):
        ...
        rabi(ssm_ge=freq[k])
        sweepparam, currentprob = readout_python(
            num_patterns=101,
            num_records=200,
            sweepparam=np.linspace(0, 100, 101),
            ave=singleave,
            ifprint=0,
            ifsave=0,
            ifplot=0,
    ...
    if ifsave:
        data = {"prob": prob, "freq": freq, "sweepparam": sweepparam}
        root = Tk()
        root.wm_attributes("-topmost", 1)
        savename = asksaveasfile(
            initialdir="C:\\Data\\2020\\201021_thermal_readout\\04_redFridge_test",
            parent=root,
        )
        root.destroy()
...
def freqsweep_rabi_directChange(
    freq=np.arange(4.172, 4.1725, 0.001), singleave=5, ifsave=0, ifloadsequence=1
):
...
def ramsey_ge(
    num_steps=101, ifrotphase=1, sweeptime=15000, ifload=1
):  # this is pulsed readout to ring up and ring down cavity dfor e state
    ...
    pi2_ge = Pulse(
        start=file_length - 3050 - pi2_ge_time,
        duration=-pi2_ge_time,
        amplitude=ge_amp,
        ssm_freq=ssm_ge,
        phase=0,
    )  # pulse is also a class p is an instance
    ringupdown_seq.add_sweep(
        channel=1, sweep_name="start", start=0, stop=-sweeptime, initial_pulse=pi2_ge
    )
    pi2_ge.phase = 90
    ringupdown_seq.add_sweep(
        channel=2, sweep_name="start", start=0, stop=-sweeptime, initial_pulse=pi2_ge
    )

    if ifrotphase == 0:
        pi2_ge = Pulse(
            start=file_length - 3050,
            duration=-pi2_ge_time,
            amplitude=ge_amp,
            ssm_freq=ssm_ge,
            phase=0,
...
def wx_ge(
    num_steps=101, sweeptime=30000, ifload=1
):  # this is pulsed readout to ring up and ring down cavity dfor e state
    totlength = sweeptime
    file_length = 10000 * (int(totlength / 10000) + 1)
    ...
    ringupdown_seq.add_sweep(
        channel=1, sweep_name="start", start=0, stop=-sweeptime, initial_pulse=pi_ge
    )
    pi_ge.phase = 0
    ringupdown_seq.add_sweep(
        channel=2, sweep_name="start", start=0, stop=-sweeptime, initial_pulse=pi_ge
    )

    # trigger pulse to open switch gate
    gate_trigger = Pulse(
        start=file_length - readout_dur, duration=readout_dur, amplitude=1
    )  # -500
    ringupdown_seq.add_sweep(
        channel=1, marker=2, sweep_name="none", initial_pulse=gate_trigger
    )

    # Readout
    main_pulse = Pulse(
        start=file_length - readout_dur - 500,
        duration=readout_dur,
        amplitude=readout_amp,
    )  # original readout_amp=1, duration = 1000     -1000
    ringupdown_seq.add_sweep(
        channel=4, sweep_name="none", initial_pulse=main_pulse
    )  # , marker=2

    ## markers
    alazar_trigger = Pulse(
        start=file_length - readout_dur - 1000 - 500, duration=1000, amplitude=1
    )
    ringupdown_seq.add_sweep(
        channel=3, marker=1, sweep_name="none", initial_pulse=alazar_trigger
    )
    ...
    if ifload:
        ...
        hardware_control.wx_programs.wx_set_and_amplitude_and_offset(
            amp=[1.5, 1.358, 1.5, 1.5], offset=[-0.076, 0.025, 0.09, -0.086]
        )
...
def repeat_ramsey_ge(repnum=20, ifsave=0, ifrunsequence=1):
    ...
    for k in np.arange(repnum):
        sweepparam, prob = readout_python(
            num_patterns=101,
            num_records=200,
            sweepparam=np.linspace(0, sweeptime * 1e-3, 101),
            ave=1,
            ifprint=0,
            ifsave=0,
            ifplot=0,
    ...
    if ifsave:
        ...
        root.wm_attributes("-topmost", 1)
        savename = asksaveasfile(
            initialdir="C:\\Data\\2020\\201021_thermal_readout\\03_single_photon",
            parent=root,
        )
        root.destroy()
        io.savemat(savename.name, data)
...
def T1_ge(
    num_steps=101, sweeptime=15000, ssm_ge=-0.15, pi_ge_time=20, q=0, ifload=1, ROIF=0
):  # this is pulsed readout to ring up and ring down cavity dfor e state
    totlength = sweeptime + 4000
    file_length = 10000 * (int(np.ceil(totlength / 10000)) + 1)
    ...
    if q == 0:  # qubit 2
        ...
        ringupdown_seq.add_sweep(
            channel=4, sweep_name="start", start=0, stop=-sweeptime, initial_pulse=pi_ge
        )
    ...
    if q == 0:
        ...
        ringupdown_seq.add_sweep(
            channel=2, sweep_name="none", initial_pulse=main_pulse
        )  # , marker=2
...
def T1_ge_modulation(
    ...
    if q == 1:  # qubit 1
        pi_ge = Pulse(
            start=file_length - readout_dur - 200,
            duration=-pi_ge_time,
            amplitude=ge_amp,
            ssm_freq=ssm_ge,
            phase=0,
        ...
        ringupdown_seq.add_sweep(
            channel=3, sweep_name="start", start=0, stop=-sweeptime, initial_pulse=pi_ge
        )

        mod_pulse = Pulse(
            start=file_length - readout_dur - 100, amplitude=1, duration=0, phase=0
        )
        ringupdown_seq.add_sweep(
            channel=3,
            marker=2,
...
def T1_ge_Raman_2q_RO(
    ...
    ssm_Q2 = (
        ssm_ge + detun + 0.001
    )  # +0.001 for amp_Q2=0.5 # excitation -detun; decay + detun
    ...
    if q == 0:  # qubit 2
        ...
        the_seq.add_sweep(
            ...
            sweep_name="width",
            start=-pulse_len_add,
            stop=-coaxtime - pulse_len_add,
            initial_pulse=coax_drive,
        )
        #        coax_drive.phase = 90
        ...
        the_seq.add_sweep(
            ...
            start=0,
            stop=-coaxtime,
            initial_pulse=q2_off_resonance_drive,
        )
        ...
        if ifsideband == 1:
            for k in np.arange(num_steps):
                if (
                    k >= 0
                ):  # Note (WC): The first pulse with nonzero duration should larger than 2*width
                    ...
                    pulseLength = np.int_(
                        sweeptime * k / (num_steps - 1)
                    )  # for qubit drive
                    pulseLength2 = np.int_(
                        pulse_len_add + sweeptime * k / (num_steps - 1)
                    )  # for RO cavity drive
                    pulseStart = (
                        file_length - readout_dur - pulseLength
                    )  # file_length-pi_ge_time-pulseLength - 1000
                    pulseStart2 = (
                        file_length - readout_dur - pulseLength2
                    )  # file_length-pi_ge_time-pulseLength - 1000
                    ...
                    # gaussian shape
                    pdata1[0:width] = amp_Q2 * np.exp(
                        -((tdata[0:width] - width) ** 2) / (2 * sigma**2)
                    )
                    pdata2[0:width] = amp_coax * np.exp(
                        -((tdata2[0:width] - width) ** 2) / (2 * sigma**2)
                    )
                    ...
                    secondStop = np.int_(-width + sweeptime * k / (num_steps - 1))
                    secondStop2 = np.int_(
                        -width + +pulse_len_add + sweeptime * k / (num_steps - 1)
                    )

                    pdata1[width:secondStop] = amp_Q2
                    pdata2[width:secondStop2] = amp_coax

                    pdata1[secondStop:pulseLength] = amp_Q2 * np.exp(
                        -((tdata[secondStop:pulseLength] - secondStop) ** 2)
                        / (2 * sigma**2)
                    )
                    pdata2[secondStop2:pulseLength2] = amp_coax * np.exp(
                        -((tdata2[secondStop2:pulseLength2] - secondStop2) ** 2)
                        / (2 * sigma**2)
                    )

                    #                    the_seq.channel_list[2][0][k,pulseStart:pulseStart+pulseLength] *= pdata1
                    the_seq.channel_list[3][0][
                        k, pulseStart : pulseStart + pulseLength
                    ] *= pdata1

                    the_seq.channel_list[0][0][
                        k, pulseStart2 : pulseStart2 + pulseLength2
                    ] *= pdata2
    ...
    main_pulse = Pulse(
        start=file_length - readout_dur,
        duration=readout_dur,
        amplitude=1.3 * readout_amp_1 * 0 + 0.035,
        ssm_freq=ROIF1,
        phase=-file_length * ROIF1 * 360,
    )
    ...
    main_pulse = Pulse(
        start=file_length - readout_dur,
        duration=readout_dur,
        amplitude=1.3 * readout_amp_2 * 0 + 0.035,
        ssm_freq=ROIF2,
        phase=-file_length * ROIF2 * 360,
    )
    the_seq.add_sweep(channel=1, sweep_name="none", initial_pulse=main_pulse)
...
def T1_M_ge_2q_RO(
    ...
    if q == 0:  # qubit 2
        pi_ge = Pulse(
            start=file_length - readout_dur - pre_readout_dur,
            duration=-pi_ge_time,
            amplitude=ge_amp,
            ssm_freq=ssm_ge,
            phase=0,
    ...
    main_pulse = Pulse(
        start=file_length - readout_dur - pre_readout_dur,
        duration=pre_readout_dur,
        amplitude=1.3 * readout_amp,
        ssm_freq=ROIF1,
        phase=0,
    )
    ringupdown_seq.add_sweep(
...
def T1_ef(
    ...
    if q == 0:  # 0 for qubit 2
        pi_ge = Pulse(
            start=file_length - readout_dur - 0 * pi_ge_time - 1 * pi_ef_time - 50,
            duration=-pi_ge_time,
            amplitude=ge_amp,
            ssm_freq=ssm_ge,
            phase=0,
        ...
        pi_ef = Pulse(
            start=file_length - readout_dur - 0 * pi_ge_time - 0 * pi_ef_time - 50,
            duration=-pi_ef_time,
            amplitude=ef_amp,
            ssm_freq=ssm_ef,
            phase=0,
        ...
        ringupdown_seq.add_sweep(
            channel=1, sweep_name="start", start=0, stop=-sweeptime, initial_pulse=pi_ef
        )
        pi_ef.phase = 90 + phase_offset_ef
        ringupdown_seq.add_sweep(
            channel=2, sweep_name="start", start=0, stop=-sweeptime, initial_pulse=pi_ef
        )
    ...
    if q == 1:  # 0 for qubit 1
        ...
        ringupdown_seq.add_sweep(
            channel=4, sweep_name="start", start=0, stop=-sweeptime, initial_pulse=pi_ef
        )
...
def T1_ge_modified(
    num_steps=101, sweeptime=15000, ifload=1
):  # this is pulsed readout to ring up and ring down cavity dfor e state
    totlength = sweeptime + 4000
    file_length = 10000 * (int(totlength / 10000) + 1)
    ...
    pi_ge = Pulse(
        start=file_length - readout_dur - 50 - 5,
        duration=-pi_ge_time,
        amplitude=ge_amp,
        ssm_freq=ssm_ge,
        phase=0,
    ...
    ringupdown_seq.add_sweep(
        channel=1, sweep_name="width", start=0, stop=-sweeptime, initial_pulse=pi_ge
    )
    pi_ge.phase = 90 + phase_offset
    ringupdown_seq.add_sweep(
        channel=2, sweep_name="width", start=0, stop=-sweeptime, initial_pulse=pi_ge
    )
...
def ch12_leakage(
    ...
    the_seq = Sequence(
        file_length, num_steps
    )  # this creates something called the_seq that is an instance of a sequence class
    ## channels
    #    pi_ge=34
    #    pi_ef=28
    #    pi_hf=50
    ...
    p2_pi_ge = Pulse(
        start=7000, duration=-pulse_time, amplitude=1, ssm_freq=ssb, phase=0
    )  # pulse is also a class p is an instance
    the_seq.add_sweep(
        channel=1, sweep_name="start", start=0, stop=-pulse_time, initial_pulse=p2_pi_ge
    )
    p2_pi_ge.phase = 90 + offset
    the_seq.add_sweep(
        channel=2, sweep_name="start", start=0, stop=-pulse_time, initial_pulse=p2_pi_ge
    )
    ...
    if True:
        ...
        channel4_ch = the_seq.channel_list[3][0]
        plt.imshow(
            channel2_ch[0:200, 6000:7000], aspect="auto", extent=[6000, 7000, 200, 0]
        )
        plt.show()
...
def pi_nopi(
    coef=0, coefpief=0, off=0, pi_ge=24, pi_ef=20, ssm_ge=-0.04575, q=0, r=0, ROIF=0.01
):  # this is pulsed readout to ring up and ring down cavity dfor e state
    ...
    if q == 1:
        pi_ge = Pulse(
            start=file_length - readout_dur - coefpief * pi_ef,
            duration=-(pi_ge) * coef,
            amplitude=ge_amp,
            ssm_freq=ssm_ge,
            phase=0,
        ...
        ringupdown_seq.add_sweep(
            channel=4, sweep_name="none", start=0, stop=-sweep_time, initial_pulse=pi_ge
        )

        # pi_ef pulse
        pi_ef = gen.Pulse(
            start=file_length - readout_dur,
            duration=-pi_ef * coefpief,
            amplitude=ge_amp,
            ssm_freq=ssm_ef,
            phase=0,
        )
        ringupdown_seq.add_sweep(
            channel=4, sweep_name="none", start=0, stop=-sweep_time, initial_pulse=pi_ef
        )
    # if q==0:
...
def pi_nopi_noise(
    coef=0, off=0, pi_ge=24, ssm_ge=-0.04575, q=0, r=0, ROIF=0.01
):  # this is pulsed readout to ring up and ring down cavity dfor e state
    # (coef=0,coefpief=0,off=0,ro_dur=8000,ro_amp=1,pi_ge=24,pi_ef=20,ssm_ge = -0.04575,ssm_ef=-0.04575-0.15)
    ...
    if q == 1:
        ...
        noise_pulse = Pulse(
            start=file_length - readout_dur, amplitude=1, duration=0, phase=0
        )
        ringupdown_seq.add_sweep(
            channel=1,
            marker=1,
...
def nopi_plus_minus(
    ...
    # pi_ge pulse
    rabi_ge = Pulse(
        start=file_length - readout_dur - coefES * pi_ef,
        duration=-pi_ge * coef,
        amplitude=ge_amp,
        ssm_freq=ssm_ge,
        phase=0,
    ...
    ringupdown_seq.add_sweep(
        channel=1, sweep_name="none", start=0, stop=-sweep_time, initial_pulse=rabi_ge
    )
    rabi_ge.phase = 90 + phase_offset
    ringupdown_seq.add_sweep(
        channel=2, sweep_name="none", start=0, stop=-sweep_time, initial_pulse=rabi_ge
    )

    # pi_ef/2 pulse to prepare |+-> eigenstate
    rabi_ef = Pulse(
        start=file_length - (pi_ef / 2) * coefES - readout_dur,
        duration=-(pi_ef / 2) * coefES,
        amplitude=amp_ef,
        ssm_freq=ssm_ef,
        phase=0 + es_phase,
    )  # pulse is also a class p is an instance
    ringupdown_seq.add_sweep(
        channel=1, sweep_name="none", start=0, stop=-sweep_time, initial_pulse=rabi_ef
    )
    rabi_ef.phase = 90 + phase_offset_ef + es_phase
    ringupdown_seq.add_sweep(
        channel=2, sweep_name="none", start=0, stop=-sweep_time, initial_pulse=rabi_ef
    )
...
##END geom
def pipi_pi_nopi(
    coef=0, off=0, ro_dur=8000, ro_amp=1, pi_ge=24, ssm_ge=-0.04575
):  # this is pulsed readout to ring up and ring down cavity dfor e state
    file_length = 16000
    num_steps = 3
    ringupdown_seq = Sequence(
...
def n_pi_2(
    coef=0, off=0, ro_dur=8000, ro_amp=1, pi_ge=24, pi_ef=20, ssm_ge=-0.04575, q=1
):  # this is pulsed readout to ring up and ring down cavity dfor e state
    file_length = 16000
    num_steps = 2
    ringupdown_seq = Sequence(
    ...
    if q == 0:  # qubit 2
        pi_2x = Pulse(
            start=file_length - readout_dur - 5,
            duration=-(pi_ge / 2) * coef,
            amplitude=ge_amp,
            ssm_freq=ssm_ge,
            phase=90,
        )  # pulse is also a class p is an instance
        ringupdown_seq.add_sweep(
            channel=3, sweep_name="none", start=0, stop=-sweep_time, initial_pulse=pi_2x
        )
...
def x_measurement(
    num_steps=51, sweep_time=200, ssm_ge=-0.15, q=0, pi_ge_time=50
):  # this is pulsed readout to ring up and ring down cavity dfor e state
    file_length = 16000
    #    num_steps = 51
    ringupdown_seq = Sequence(
    ...
    if q == 0:  # qubit 2
        rabi_ge = Pulse(
            start=file_length - readout_dur - pi_ge_time - sweep_time - 5,
            duration=0,
            amplitude=ge_amp,
            ssm_freq=ssm_ge,
            phase=0,
        ...
        pi_2x = Pulse(
            start=file_length - readout_dur - pi_ge_time / 2 - 5,
    ...
    if q == 1:  # qubit 1
        rabi_ge = Pulse(
            start=file_length - readout_dur - pi_ge_time / 2 - 10,
            duration=0,
            amplitude=ge_amp,
            ssm_freq=ssm_ge,
            phase=0,
...
def y_measurement(
    ...
    if q == 0:  # qubit 2
        rabi_ge = Pulse(
            start=file_length - readout_dur - pi_ge_time - sweep_time - 200,
            duration=0,
            amplitude=ge_amp,
            ssm_freq=ssm_ge,
            phase=0,
        ...
        pi_2x = Pulse(
            start=file_length - readout_dur - pi_ge_time / 2 - 100,
            duration=pi_ge_time / 2,
            amplitude=ge_amp,
            ssm_freq=ssm_ge,
            phase=180,
...
def rabi_ge(
    num_steps=51, sweep_time=200, ssm_ge=-0.15, ROIF1=0, ROIF2=0, q=0
):  # this is pulsed readout to ring up and ring down cavity dfor e state
    file_length = 16000
    #    num_steps = 51
    ringupdown_seq = Sequence(
...
def vacuum_rabi(
    num_steps=51, sweep_time=200, ssm_ge=-0.15, ROIF1=0, ROIF2=0, q=0, pi_ge_time=50
):  # this is pulsed readout to ring up and ring down cavity dfor e state
    file_length = 16000
    #    num_steps = 51
    ringupdown_seq = Sequence(
    ...
    if q == 0:  # qubit 2
        ...
        coupler = Pulse(
            start=file_length - readout_dur - 100, duration=0, amplitude=coupler_amp
        )
        ringupdown_seq.add_sweep(
            channel=2,
            sweep_name="width",
...
def teaching_test(
    num_steps=51, sweep_time=200, pi_ge_time=49, ssm_ge=-0.15, ROIF2=0, q=0
):  # this is pulsed readout to ring up and ring down cavity dfor e state
    #    file_length = 16000
    ...
    if q == 0:  # qubit 2
        pi_ge = Pulse(
            start=file_length - readout_dur - pi_ge_time,
            duration=-pi_ge_time / 2,
            amplitude=ge_amp,
            ssm_freq=ssm_ge,
            phase=0,
        ...
        pi_ge = Pulse(
            start=file_length - readout_dur - pi_ge_time / 2,
            duration=-pi_ge_time / 2,
            amplitude=ge_amp,
            ssm_freq=ssm_ge,
            phase=0,
        ...
        ringupdown_seq.add_sweep(
            channel=4, sweep_name="phase", start=0, stop=4 * 360, initial_pulse=pi_ge
        )
    ...
    if q == 1:  # qubit 1
        ...
        ringupdown_seq.add_sweep(
            channel=3, sweep_name="phase", start=0, stop=4 * 360, initial_pulse=pi_ge
        )
...
def rabi_ge_stest(
    num_steps=51, sweep_time=200, ssm_ge=-0.15, q=0
):  # this is pulsed readout to ring up and ring down cavity dfor e state
    file_length = 16000
    #    num_steps = 51
    ringupdown_seq = Sequence(
...
def Always_On(
    ...
    # Readout
    main_pulse = Pulse(
        start=100, duration=file_length - 100, amplitude=1
    )  # original readout_amp=1, duration = 1000     -1000
    ringupdown_seq.add_sweep(
        channel=1, marker=2, sweep_name="none", initial_pulse=main_pulse
    )  # , marker=2
...
def rabi_ge_2qubit(
    num_steps=51, sweep_time=200, ssm_ge1=-0.15, ssm_ge2=-0.15, ROIF1=0.1, ROIF2=0.1
):  # for herterodyne
    file_length = 16000
    #    num_steps = 51
    ringupdown_seq = Sequence(
    ...
    ringupdown_seq.add_sweep(
        channel=3, sweep_name="width", start=0, stop=-sweep_time, initial_pulse=rabi_ge
    )
    #
    # qubit 2 rabi
    rabi_ge = Pulse(
    ...
    ringupdown_seq.add_sweep(
        channel=4, sweep_name="width", start=0, stop=-sweep_time, initial_pulse=rabi_ge
    )
    ...
    main_pulse = Pulse(
        ...
        duration=readout_dur,
        amplitude=1.3 * readout_amp,
        ssm_freq=-file_length * ROIF1 * 360,
        phase=0,
    )
    ringupdown_seq.add_sweep(channel=1, sweep_name="none", initial_pulse=main_pulse)
    # main_pulse.phase = 90
    ...
    main_pulse = Pulse(
        ...
        duration=readout_dur,
        amplitude=1.3 * readout_amp,
        ssm_freq=-file_length * ROIF2 * 360,
        phase=0,
    )
    ringupdown_seq.add_sweep(channel=1, sweep_name="none", initial_pulse=main_pulse)
    # main_pulse.phase = 90
...
def rabi_ef(
    ...
    if q == 1:  # qubit 1
        # first pi_ge pulse
        pi_ge_pulse = Pulse(
            start=file_length - readout_dur - pi_ge - buffer,
            duration=-pi_ge,
            amplitude=ge_amp,
            ssm_freq=ssm_ge,
            phase=0,
...
##END geom
def ef_test(
    num_steps=51, sweep_time=200, pi_ge=20, ssm_ge=-0.150, ssm_ef=-0.300, ef_amp=1, q=0
):
    file_length = 30000
    #    num_steps = 101
    ringupdown_seq = Sequence(
    ...
    if q == 1:  # qubit 1
        ...
        ringupdown_seq.add_sweep(
            channel=3, sweep_name="none", initial_pulse=pi_ge_pulse
        )
        pi_ge_pulse.phase = 90 + phase_offset
        ringupdown_seq.add_sweep(
            channel=4, sweep_name="none", initial_pulse=pi_ge_pulse
    ...
    ringupdown_seq.add_sweep(
        channel=1, marker=1, sweep_name="none", initial_pulse=main_pulse
    )  # , marker=2
...
def rabi_J(
    ...
    if q == 0:  # qubit 2
        # first pi_ge pulse
        pi_ge_pulse = Pulse(
            start=file_length - readout_dur - 0 * pi_ge - 1 * pi_ef - buffer,
            duration=-pi_ge,
            amplitude=ge_amp,
            ssm_freq=ssm_ge,
            phase=0,
        ...
        # first pi_ef pulse
        pi_ge_pulse = Pulse(
            start=file_length - readout_dur - 0 * pi_ge - buffer,
            duration=-pi_ef,
            amplitude=ge_amp,
            ssm_freq=ssm_ef,
            phase=0,
...
def rabi_ef_ES(
    ...
    # first pi-pulse
    pi_pulse = Pulse(
        start=file_length - readout_dur - pi_ef / 2 - 3 * 5,
        duration=-pi_ge,
        amplitude=amp_ge,
        ssm_freq=ssm_ge,
        phase=0,
    ...
    the_seq.add_sweep(
        channel=1, sweep_name="start", start=0, stop=-rabi_time, initial_pulse=pi_pulse
    )
    pi_pulse.phase = 90 + orth_ph
    the_seq.add_sweep(
        channel=2, sweep_name="start", start=0, stop=-rabi_time, initial_pulse=pi_pulse
    )

    # pulse to prepare eigenstate
    pi_pulse = Pulse(
        start=file_length - readout_dur - 2 * 5,
        duration=-pi_ef / 2,
        amplitude=amp_ef,
        ssm_freq=ssm_ef,
        phase=0 + ph,
    ...
    the_seq.add_sweep(
        channel=1, sweep_name="width", start=0, stop=-rabi_time, initial_pulse=rabief
    )
    rabief.phase = 180 + orth_ph_ef
    the_seq.add_sweep(
        channel=2, sweep_name="width", start=0, stop=-rabi_time, initial_pulse=rabief
    )
...
def dynamics_variables_EP(
    ...
    # first pi-pulse
    pi_pulse = Pulse(
        start=file_length - readout_dur - pi_tomo / 2 - pi_ES / 2 - 4 * 5,
        duration=-pi_ge,
        amplitude=amp_ge,
        ssm_freq=ssm_ge,
        phase=0,
    ...
    pi_pulse = Pulse(
        start=file_length - readout_dur - pi_tomo / 2 - 3 * 5,
        duration=-pi_ES / 2,
        amplitude=amp_ES,
        ssm_freq=ssm_ef,
        phase=0 + phase_es,
    ...
    # variable pulse
    pt_pi_ge_r = Pulse(
        start=file_length - readout_dur - pi_tomo / 2 - 2 * 5,
        duration=0,
        amplitude=amp_max,
        ssm_freq=ssm_ef + delta_ef,
        phase=90,
        phase_ini=ph_ini,
        t_loop=period,
        ff=1,
        detun_NH=detun,
        jmin=amp_factor,
    )  # , phase_ini=np.pi/2, t_loop=400, ff=1) #pulse is also a class p is an instance
    ringupdown_seq.add_sweep(
        channel=1,
        sweep_name="width",
        start=0,
        stop=-rabi_time,
        initial_pulse=pt_pi_ge_r,
    )
    ...
    p2_pi_ge = Pulse(
        ...
        ff=None,
        detunlinear=0,
        detun_NH_phase=detun,
    )  # , phase_ini=np.pi/2, t_loop=400, ff=1) #pulse is also a class p is an instance
    ringupdown_seq.add_sweep(
        channel=1,
        sweep_name="detuned_phase",
        start=0,
        stop=-rabi_time,
        initial_pulse=p2_pi_ge,
    )
...
def const_Jdelta_drive_EP(
    ...
    ringupdown_seq.add_sweep(
        channel=1, sweep_name="start", start=0, stop=-rabi_time, initial_pulse=p2_pi_ge
    )
    p2_pi_ge.phase = 90 + phase_tomo + orth_ph_ef
    ringupdown_seq.add_sweep(
        channel=2, sweep_name="start", start=0, stop=-rabi_time, initial_pulse=p2_pi_ge
    )
...
def spectroscopy_ge(
    num_steps, ssm_start=-0.05, ssm_stop=-0.15, spec_amp=0.5, ROIF1=0, ROIF2=0, q=0
):  # this is pulsed readout to ring up and ring down cavity dfor e state
...
def spectroscopy_pump(
    ...
    if q == 1:  # qubit 1
        ...
        ringupdown_seq.add_sweep(
            ...
            start=ssm_start,
            stop=ssm_stop,
            initial_pulse=pump_pulse,
        )

    if q == 0:  # qubit 2

        pi_ge = Pulse(
            start=file_length - readout_dur - 720,
            duration=-59.9431,
            amplitude=ge_amp,
            ssm_freq=-0.111,
            phase=0,
        ...
        ringupdown_seq.add_sweep(
            channel=4, sweep_name="none", start=0, stop=0, initial_pulse=pi_ge
        )
...
def tomo_pump(
    ...
    pi_ge = Pulse(
        start=file_length - readout_dur - pump_time - 20,
        duration=-pi_ge_time_q1 * pi_q1,
        amplitude=ge_amp,
        ssm_freq=ssm_geq1,
        phase=0,
    )  # pulse is also a class p is an instance
    ...
    pi_ge_2 = Pulse(
        start=file_length - readout_dur - pump_time - 20,
        duration=-pi_ge_time_q2 * pi_q2,
        amplitude=ge_amp,
        ssm_freq=ssm_geq2,
        phase=0,
    )  # pulse is also a class p is an instance
    ringupdown_seq.add_sweep(
        channel=4, sweep_name="none", start=0, stop=0, initial_pulse=pi_ge_2
    )
    # qubit freq
    # rabi_ge = Pulse(start=file_length-readout_dur-10, duration=-10000, amplitude=ge_amp, ssm_freq=ssm_ge, phase=0)
    # ringupdown_seq.add_sweep(channel=4, sweep_name='none', initial_pulse=rabi_ge)
...
##END geom
def pump_versus_time(
    num_steps, spec_amp=0.5, ROIF1=0, ROIF2=0, q=0, pump_freq=-0.075, pulse_time=500
):  # this is pulsed readout to ring up and ring down cavity dfor e state
    file_length = 16000  # ns
    ...
    if q == 0:  # qubit 2
        ...
        pi_ge = Pulse(
            start=file_length - readout_dur - 20,
            duration=-148,
            amplitude=ge_amp,
            ssm_freq=-0.110,
            phase=0,
...
def spectroscopy_ge_modulation(
    num_steps=101, ssm_start=-0.05, ssm_stop=-0.15, spec_amp=0.5, ROIF1=0, ROIF2=0, q=0
):  # this is pulsed readout to ring up and ring down cavity dfor e state
    file_length = 16000  # ns
    #    num_steps = 51
    ringupdown_seq = Sequence(
    ...
    if q == 1:  # qubit 1
        ...
        mod_pulse = Pulse(
            start=file_length - readout_dur - 10, amplitude=1, duration=-10000, phase=0
        )
        ringupdown_seq.add_sweep(
            channel=3, marker=2, sweep_name="none", initial_pulse=mod_pulse
        )
...
def spectroscopy_ge_DLnoise(
    ...
    if q == 1:  # qubit 1
        ...
        ringupdown_seq.add_sweep(
            channel=1, marker=1, sweep_name="none", initial_pulse=noise_pulse
        )
        mod_pulse = Pulse(
            start=file_length - readout_dur - 10, amplitude=1, duration=-10000, phase=0
        )
...
def spectroscopy_ge_coupler_switch(
    num_steps=101,
    ssm_start=-0.05,
    ssm_stop=-0.15,
    spec_amp=0.5,
    ...
    if q == 0:  # qubit 2
        coupler = Pulse(
            start=file_length - readout_dur - sweep_time_ssm - time_coupler,
            duration=+readout_dur + sweep_time_ssm + time_coupler,
            amplitude=coupler_amp,
        )
        ringupdown_seq.add_sweep(channel=2, sweep_name="none", initial_pulse=coupler)

        rabi_ge = Pulse(
            start=file_length - readout_dur - 10,
            duration=-sweep_time_ssm,
            amplitude=spec_amp,
            ssm_freq=0,
            phase=0,
        )  # pulse is also a class p is an instance
...
def spectroscopy_ROcavity(
    ...
    main_pulse = Pulse(
        start=file_length - readout_dur,
        duration=readout_dur,
        amplitude=1.3 * readout_amp_1 * spec_amp,
        ssm_freq=ROIF1,
        phase=-file_length * ROIF1 * 360,
    )
    the_seq.add_sweep(channel=1, sweep_name="none", initial_pulse=main_pulse)
    ...
    main_pulse = Pulse(
        start=file_length - readout_dur,
        duration=readout_dur,
        amplitude=1.3 * readout_amp_2 * spec_amp,
        ssm_freq=ROIF2,
        phase=-file_length * ROIF2 * 360,
    )
    the_seq.add_sweep(channel=1, sweep_name="none", initial_pulse=main_pulse)
...
def parametric_coupling_time_domain_Raman_Q2(
    num_steps=101,
    ssm_ge_Q1=-0.2,
    ssm_ge_Q2=-0.2,
    pi_ge_Q1=20,
    ...
    # qubit 1 pi pulse
    pi_ge = Pulse(
        start=file_length - readout_dur - 400,
        duration=-pi_ge_Q1,
        amplitude=ge_amp,
        ssm_freq=ssm_ge_Q1,
        phase=0,
    ...
    main_pulse = Pulse(
        start=file_length - readout_dur,
        duration=readout_dur,
        amplitude=1.3 * readout_amp_1 * 0 + 0.035 * 1.525,
        ssm_freq=ROIF1,
        phase=-file_length * ROIF1 * 360,
    )
    the_seq.add_sweep(channel=1, sweep_name="none", initial_pulse=main_pulse)
...
def parametric_coupling_Q1Ramsey_Raman_Q2(
    num_steps=101,
    ssm_ge_Q1=-0.2,
    ssm_ge_Q2=-0.2,
    pi_ge_Q1=20,
    ...
    # off-resonance drive for RO (of Q2)
    coax_drive = Pulse(
        start=file_length - readout_dur - pi_ge_Q1 / 2 - 10,
        duration=0,
        amplitude=amp_coax,
        ssm_freq=ssm_coax,
        phase=0,
    ...
    # qubit 1 pi/2 pulse
    pi_ge = Pulse(
        start=file_length - readout_dur - pi_ge_Q1 / 2 - 100,
        duration=-pi_ge_Q1 / 2,
        amplitude=ge_amp,
        ssm_freq=ssm_ge_Q1,
        phase=0,
    ...
    parametric_12 = Pulse(
        start=file_length - readout_dur - 20 - pi_ge_Q1 / 2,
        duration=0,
        amplitude=spec_amp,
        ssm_freq=ssm_para,
        phase=0,
...
def e_char(
    ...
    ringupdown_seq.add_sweep(
        channel=1, sweep_name="width", start=0, stop=-sweep_time, initial_pulse=rabi_ef
    )
    rabi_ef.phase = 90
    ringupdown_seq.add_sweep(
        channel=2, sweep_name="width", start=0, stop=-sweep_time, initial_pulse=rabi_ef
    )
...
def rabi_ge_pi_2():  # this is pulsed readout to ring up and ring down cavity dfor e state
    ...
    pi_ge = Pulse(
        start=file_length - readout_dur - 2050,
        duration=-pi_ge / 2,
        amplitude=ge_amp,
        ssm_freq=ssm_ge,
        phase=0,
    ...
    ringupdown_seq.add_sweep(
        channel=1, sweep_name="width", start=0, stop=-sweep_time, initial_pulse=pi_ge
    )
    pi_ge.phase = 90
    ringupdown_seq.add_sweep(
        channel=2, sweep_name="width", start=0, stop=-sweep_time, initial_pulse=pi_ge
    )
    ...
    ringupdown_seq.add_sweep(
        channel=1, sweep_name="width", start=0, stop=-sweep_time, initial_pulse=rabi_ge
    )
    rabi_ge.phase = 90
    ringupdown_seq.add_sweep(
        channel=2, sweep_name="width", start=0, stop=-sweep_time, initial_pulse=rabi_ge
    )

    # Readout
    main_pulse = Pulse(
        start=file_length - readout_dur - 1000,
        duration=readout_dur,
        amplitude=readout_amp,
    )  # original readout_amp=1, duration = 1000
    ringupdown_seq.add_sweep(
        channel=1, marker=2, sweep_name="none", initial_pulse=main_pulse
    )
...
def ramsey(
    ...
    ringupdown_seq = Sequence(
        file_length, num_steps
    )  # this creates something called ringupdown_seq that is an instance of a sequence class
    ## channels
    #    pi_ge=17
    #    t1_time = 1000
    #    ssm_ge = 0.3885
    if q == 0:  # qubit 2
        ...
        t2_ge = Pulse(
            start=file_length - readout_dur - buffer - pi_ge / 2 - pi_echo_coef * pi_ge,
            duration=-pi_ge / 2,
            amplitude=ge_amp,
            ssm_freq=ssm_ge,
            phase=0,
        ...
        t2_ge = Pulse(
            start=file_length - readout_dur - buffer - pi_ge / 2,
            duration=-pi_ge * pi_echo_coef,
            amplitude=ge_amp,
            ssm_freq=ssm_ge,
            phase=0,
        )  # pulse is also a class p is an instance
    ...
    main_pulse = Pulse(
        ...
        ssm_freq=ROIF,
        phase=-file_length * ROIF * 360,
    )  # amplitude= 1.3*readout_amp_1,ssm_freq=ROIF1, phase=-file_length*ROIF1*360
    ringupdown_seq.add_sweep(channel=2, sweep_name="none", initial_pulse=main_pulse)
...
def ramsey_quantum_effiency(
    num_steps=101, ssm_ge=-0.15, pi_ge=20, ROIF=0, q=0, RO_ram_amp=0.0
):  # this is pulsed readout to ring up and ring down cavity dfor e state
    #    file_length = 10000*(int(t1_time/10000)+1) #64000
    # totlength = t1_time + 4000
    # file_length = 10000 * (int(totlength/10000)+2)
    ...
    if q == 0:  # qubit 2
        ...
        ringupdown_seq.add_sweep(
            channel=4, sweep_name="phase", start=0, stop=360, initial_pulse=t2_ge
        )
    #        t2_ge.phase = 90+phase_offset
    #        ringupdown_seq.add_sweep(channel=2,  sweep_name='start', start=0, stop=-t1_time/2,initial_pulse=t2_ge)
    #         if osc !=0:
...
def ramsey_modulation(
    ...
    if q == 1:  # qubit 1
        ...
        ringupdown_seq.add_sweep(
            channel=3, sweep_name="start", start=0, stop=-t1_time, initial_pulse=t2_ge
        )

        mod_pulse = Pulse(
            start=file_length - readout_dur - buffer / 2 - pi_ge / 2,
            amplitude=1,
            duration=0,
            phase=0,
        )
...
def ramsey_modulation_step(
    ...
    if q == 1:  # qubit 1
        readout_amp = readout_amp_1
        t2_ge = Pulse(
            start=file_length - readout_dur - buffer - pi_ge / 2 - t1_time,
            duration=-pi_ge / 2,
            amplitude=ge_amp,
            ssm_freq=ssm_ge,
            phase=0,
        ...
        mod_pulse = Pulse(
            start=file_length - readout_dur - buffer / 2 - pi_ge / 2 - t1_time,
            amplitude=1,
            duration=0,
            phase=0,
        )
...
def ramsey_ef(
    ...
    if q == 0:  # qubit 2
        ...
        ringupdown_seq.add_sweep(
            channel=1, sweep_name="start", start=0, stop=-t1_time, initial_pulse=t2_ef
        )
        t2_ef.phase = 90 + phase_offset
        ringupdown_seq.add_sweep(
            channel=2, sweep_name="start", start=0, stop=-t1_time, initial_pulse=t2_ef
        )

        # first pi_ef/2 pulse
        t2_ef = Pulse(
            start=file_length - readout_dur - buffer - pi_ef / 2 - pi_echo_coef * pi_ef,
            duration=-pi_ef / 2,
            amplitude=ef_amp,
            ssm_freq=ssm_ef,
            phase=0,
        ...
        # echo pulse
        t2_ef = Pulse(
            start=file_length - readout_dur - buffer - pi_ef / 2,
            duration=-pi_ef * pi_echo_coef,
            amplitude=ge_amp,
            ssm_freq=ssm_ef,
            phase=0,
        )  # pulse is also a class p is an instance
    ...
    if q == 1:  # qubit 1
        ...
        ringupdown_seq.add_sweep(
            channel=3, sweep_name="start", start=0, stop=-t1_time, initial_pulse=t2_ef
        )
        t2_ef.phase = 90 + phase_offset
        ringupdown_seq.add_sweep(
            channel=4, sweep_name="start", start=0, stop=-t1_time, initial_pulse=t2_ef
        )
        ...
        # echo pulse
        t2_ef = Pulse(
            start=file_length - readout_dur - buffer - pi_ge - pi_ef / 2,
            duration=-pi_ef * pi_echo_coef,
            amplitude=ge_amp,
            ssm_freq=ssm_ef,
            phase=0,
        ...
        if osc != 0:
            t2_ef = Pulse(
                start=file_length - readout_dur - buffer - pi_ge,
                duration=-pi_ef / 2,
                amplitude=ge_amp,
                ssm_freq=ssm_ef,
                phase=0,
        ...
        ringupdown_seq.add_sweep(
            channel=3, sweep_name="none", start=0, stop=-t1_time, initial_pulse=t2_ef
        )
        t2_ef.phase = 90 + phase_offset
        ringupdown_seq.add_sweep(
            channel=4, sweep_name="none", start=0, stop=-t1_time, initial_pulse=t2_ef
        )
...
def check_f0_e1_transition_GaussianPulse(
    num_steps=101,
    rotangleEF=np.pi,
    iffinalpiGE=1,
    sweeptime=2000,
    ...
    pi_ge = Pulse(
        start=file_length - 3050 - rotDurationEF - pi_ge_time - 2 * width - 1000,
        duration=-pi_ge_time,
        amplitude=ge_amp,
        ssm_freq=ssm_ge,
        phase=0,
    ...
    rot_ef = Pulse(
        start=file_length - 3050 - pi_ge_time - 2 * width - 1000,
        duration=-rotDurationEF,
        amplitude=ef_amp,
        ssm_freq=ssm_ef,
        phase=0,
    )  # pulse is also a class p is an instance
    ringupdown_seq.add_sweep(
        channel=1, sweep_name="start", start=0, stop=-sweeptime, initial_pulse=rot_ef
    )
    rot_ef.phase = 90
    ringupdown_seq.add_sweep(
        channel=2, sweep_name="start", start=0, stop=-sweeptime, initial_pulse=rot_ef
    )
    ...
    if ifsideband == 1:
        for k in np.arange(num_steps):
            ...
            pdata[0:width] = amp * np.exp(
                -((tdata[0:width] - width) ** 2) / (2 * sigma**2)
            )

            secondStop = int(width + sweeptime * k / (num_steps - 1))
            pdata[width:secondStop] = amp
            pdata[secondStop:pulseLength] = amp * np.exp(
                -((tdata[secondStop:pulseLength] - secondStop) ** 2) / (2 * sigma**2)
            )

            ringupdown_seq.channel_list[2][0][
                k, pulseStart : pulseStart + pulseLength
            ] = pdata
            ringupdown_seq.channel_list[3][0][
                k, pulseStart : pulseStart + pulseLength
            ] = pdata
...
def meas_f0toe1_population(
    ave=5, rotangleEF=1 * np.pi, ifsideband=1, ifgaussian=1, ifsave=1, sideAmp=2
):
    ...
    if ifgaussian:
        check_f0_e1_transition_GaussianPulse(
            num_steps=num_steps,
            rotangleEF=rotangleEF,
            iffinalpiGE=1,
            sweeptime=sweeptime,
            ifload=1,
            sigma=40,
            ifsideband=ifsideband,
            sideAmp=sideAmp,
        )

    sweepparam, prob0 = readout_python(
        num_patterns=num_steps,
        num_records=200,
        sweepparam=np.linspace(0, sweeptime, num_steps),
        ave=ave,
        ifprint=0,
        ifsave=0,
        ifplot=1,
    ...
    sweepparam, prob1 = readout_python(
        num_patterns=num_steps,
        num_records=200,
        sweepparam=np.linspace(0, sweeptime, num_steps),
        ave=ave,
...
def check_ge_dephasing_amp_coherentReadout(
    num_steps=101, ifload=1, ifplotsequence=1, amp=1
):  # this is pulsed readout to ring up and ring down cavity dfor e state
    file_length = 8000
    ...
    pi2_ge = Pulse(
        start=file_length - 6050 - pi2_ge_time,
        duration=-pi2_ge_time,
        amplitude=ge_amp,
        ssm_freq=ssm_ge,
        phase=0,
    ...
    main_pulse = Pulse(
        start=file_length - 6050 - pi2_ge_time, duration=2000, amplitude=amp
    )
    ...
    ringupdown_seq.add_sweep(
        channel=1, sweep_name="phase", start=0, stop=360, initial_pulse=pi2_ge
    )
    pi2_ge.phase = 90
    ringupdown_seq.add_sweep(
        channel=2, sweep_name="phase", start=0, stop=360, initial_pulse=pi2_ge
    )
...
def check_ge_dephasing_amp_ThermalReadout(
    num_steps=101, ifload=1, ifplotsequence=1
):  # this is pulsed readout to ring up and ring down cavity dfor e state
    file_length = 8000
    ...
    main_pulse = Pulse(
        start=file_length - 6050 - pi2_ge_time, duration=2000, amplitude=thermal_amp
    )
    ringupdown_seq.add_sweep(channel=3, sweep_name="none", initial_pulse=main_pulse)
...
def check_f0_e1_transition_dephasing(
    num_steps=101,
    rotangleEF=np.pi,
    iffinalpiEF=1,
    sweeptime=2000,
    ...
    pi2_ge = Pulse(
        start=file_length - 3050 - pi2_ge_time - 3000 - rotDurationEF,
        duration=-pi2_ge_time,
        amplitude=ge_amp,
        ssm_freq=ssm_ge,
        phase=0,
    ...
    rot_ef = Pulse(
        start=file_length - 3050 - pi2_ge_time - 3000,
        duration=-rotDurationEF,
        amplitude=ef_amp,
        ssm_freq=ssm_ef,
        phase=0,
...
def check_f0_e1_transition_population(
    num_steps=101,
    GErotangle=np.pi / 2,
    iffinalpiGE=1,
    sweeptime=2000,
    ...
    pi2_ge = Pulse(
        start=file_length - 3050 - pi_ge_time - 3000,
        duration=-pi2_ge_time * int(GErotangle * 2 / np.pi),
        amplitude=ge_amp,
        ssm_freq=ssm_ge,
        phase=0,
    )  # pulse is also a class p is an instance
    ringupdown_seq.add_sweep(
        channel=1, sweep_name="start", start=0, stop=-rotTime, initial_pulse=pi2_ge
    )
    pi2_ge.phase = 90
    ringupdown_seq.add_sweep(
        channel=2, sweep_name="start", start=0, stop=-rotTime, initial_pulse=pi2_ge
    )
    ...
    ringupdown_seq.add_sweep(
        channel=1, sweep_name="width", start=0, stop=-rotTime, initial_pulse=rot_ef
    )
    rot_ef.phase = 90
    ringupdown_seq.add_sweep(
        channel=2, sweep_name="width", start=0, stop=-rotTime, initial_pulse=rot_ef
    )
...
def angleSweep_f0toe1_dephasing(
    ave=5,
    singleave=5,
    transtime=0,
    sigma=0,
    ...
    for l in np.arange(ave):
        for k in np.arange(np.size(angle)):
            ...
            check_f0_e1_transition_dephasing(
                num_steps=num_steps,
                rotangleEF=angle[k],
                iffinalpiEF=1,
                sweeptime=transtime,
            ...
            sweepparam, prob0 = readout_python(
                num_patterns=num_steps,
                num_records=200,
                sweepparam=np.linspace(0, 2 * np.pi, num_steps),
                ave=singleave,
                ifprint=0,
                ifsave=0,
                ifplot=0,
            ...
            plt.ylabel("$P_e$")
            plt.title(
                "# {} in {}".format(k + 1 + l * np.size(angle), np.size(angle) * ave)
            )
            plt.show()
...
def timeSweep_f0toe1_dephasing(
    ave=1, sigma=40, sweeptime=np.linspace(0, 2000, 51), ifsave=1, ifprint=0
):
...
def ampSweep_ge_dephasingAmp(ave=1, amp=np.arange(0.05, 1.5, 0.1), ifsave=1, ifprint=0):
    ...
    for k in np.arange(np.size(amp)):
        ...
        check_ge_dephasing_amp_coherentReadout(
            num_steps=101, ifload=1, ifplotsequence=0, amp=amp[k]
        )
    ...
    if ifsave:
        data = {"sweepparam": sweepparam, "totprob": totprob, "amp": amp}
        root = Tk()
        root.wm_attributes("-topmost", 1)
        savename = asksaveasfile(
            initialdir="C:\\Data\\2020\\201021_thermal_readout\\03_single_photon",
...
def ampSweep_ge_dephasingAmp_withave(
    ave=5, amp=np.arange(0.05, 1.41, 0.05), ifsave=1, ifprint=0
):
    ...
    for m in np.arange(ave):
        for k in np.arange(np.size(amp)):
            ...
            plt.title(
                "# {} in {}: {} amp".format(
                    k + 1 + m * np.size(amp), np.size(amp) * ave, amp[k]
                )
            )
            plt.show()
    ...
    if ifsave:
        data = {"sweepparam": sweepparam, "totprob": totprob, "amp": amp, "ave": ave}
        root = Tk()
        root.wm_attributes("-topmost", 1)
        savename = asksaveasfile(
            initialdir="C:\\Data\\2020\\201021_thermal_readout\\03_single_photon",
...
def PowerSweep_ge_dephasingAmp_withave_thermalReadout(
    ave=5, att=np.arange(40, 0 - 0.1, -2), ifsave=1, ifprint=0
):
    ...
    check_ge_dephasing_amp_ThermalReadout(
        num_steps=num_steps, ifload=1, ifplotsequence=1
    )
    ...
    for m in np.arange(ave):
        for k in np.arange(np.size(att)):
            ...
            plt.title(
                "# {} in {}: {} dB att".format(
                    k + 1 + m * np.size(att), np.size(att) * ave, att[k]
                )
            )
            plt.show()
    ...
    if ifsave:
        data = {"sweepparam": sweepparam, "totprob": totprob, "att": att, "ave": ave}
        root = Tk()
        root.wm_attributes("-topmost", 1)
        savename = asksaveasfile(
            initialdir="C:\\Data\\2020\\201021_thermal_readout\\03_single_photon",
...
def PowerSweep_ge_ReadoutSNR_withave_thermalReadout(
    ave=5, att=np.arange(40, 0 - 0.1, -2), ifsave=1, ifprint=0
):
    ...
    for m in np.arange(ave):
        for k in np.arange(np.size(att)):
            ...
            if att[k] > 15:
                _, voltage, hist_g, hist_e, _ = (
                    xr.compare_ge_readout_FT_simultaneously_nosequence(
                        num_steps=5,
                        fJPA=5.6205,
                        numrec=2000,
                        ifplot=0,
                ...
                _, voltage, hist_g, hist_e, _ = (
                    xr.compare_ge_readout_FT_simultaneously_nosequence_weight(
                        num_steps=5,
                        fJPA=5.6205,
                        numrec=2000,
                        ifplot=0,
...
def single_readout_monoThermal(
    n=1,
    totnum=5000,
    ifplot=1,
    ifprint=0,
    ...
    p = 1 / (np.pi * n) * np.exp(-(alpha**2) / n) * (2 * np.pi * alpha * step)
    p = p / np.sum(p)
    ...
    for k in np.arange(np.size(amp)):
        ...
        if repnum > 0:
            check_readout_multiplestate_differentPhase(
                measTime=2000, c_amp=amp[k], phase=phase, ifplotsequence=0
            )

            [current_ch_a_g, current_ch_a_e, current_ch_b_g, current_ch_b_e] = (
                xr.compare_ge_readout_simultaneously_noSequence(
                    num_steps=num_steps,
                    numrec=repnum,
                    ifplotSNR=0,
                    ifplotamp=0,
    ...
    for m in np.arange(np.size(phase)):
        rec_rot = xr.rotate_iq(
            phase[m] * 180 / np.pi, [ch_a_g_reshape[:, m], ch_b_g_reshape[:, m]]
        )
        ch_a_g_correct[:, m] = rec_rot[0]
        ch_b_g_correct[:, m] = rec_rot[1]

        rec_rot = xr.rotate_iq(
            phase[m] * 180 / np.pi, [ch_a_e_reshape[:, m], ch_b_e_reshape[:, m]]
        )
    ...
    lim = (
        np.max(
            np.concatenate(
                (np.abs(ch_a_g), np.abs(ch_a_e), np.abs(ch_b_g), np.abs(ch_b_e))
            )
        )
        * 1.2
    )
    ...
    if ifplot:
        ...
        ax_scatter = plt.axes()
        ax_scatter.scatter(
            ch_a_g_correct, ch_b_g_correct, s=3, c="b", marker=".", alpha=0.2
        )
        ax_scatter.scatter(ch_a_e, ch_b_e, s=3, c="r", marker=".", alpha=0.2)
    ...
    if ifplot:
        ...
        SNR = np.abs((para_e[1] - para_g[1])) / (
            (np.abs(para_g[2]) + np.abs(para_e[2])) / 2
        )
        plt.figure()
    ...
    if ifsave:
        data = {
            "ch_a_g_correct": ch_a_g_correct,
            "ch_a_e_correct": ch_a_e_correct,
            "ch_b_g_correct": ch_b_g_correct,
            "ch_b_e_correct": ch_b_e_correct,
            "ch_a_g": ch_a_g,
            "ch_a_e": ch_a_e,
            "ch_b_g": ch_b_g,
            "ch_b_e": ch_b_e,
...
def single_readout_monoThermal_FTmethod(
    n=1,
    totnum=5000,
    ifplot=1,
    ifprint=0,
...
def single_readout_monoThermal_FTmethod_getSpectrum(
    n=1,
    totnum=5000,
    ifplot=1,
    ifprint=0,
    ...
    if ifsave:
        data = {
            "totspectrum_gstate": totspectrum_gstate,
...
def AmpSweep_ge_ReadoutSNR_withave_monoThermalReadout(
    ave=5, photonnum=np.arange(0.2, 8 + 0.3, 0.5), ifsave=1
):
    ...
    for m in np.arange(ave):
        for k in np.arange(np.size(photonnum)):
            ...
            [voltage_amp, hist_g_amp, hist_e_amp, voltage, hist_g, hist_e] = (
                single_readout_monoThermal(
                    n=photonnum[k],
                    totnum=5000,
                    ifplot=0,
                    ifprint=0,
            ...
            plt.title(
                "# {} in {}: {} photonnum".format(
                    k + 1 + m * np.size(photonnum),
                    np.size(photonnum) * ave,
                    photonnum[k],
                )
            )
            plt.show()
    ...
    if ifsave:
        data = {
            "totvoltage_amp": totvoltage_amp,
            "tothist_g_amp": tothist_g_amp,
            "tothist_e_amp": tothist_e_amp,
            "totvoltage_correct": totvoltage_correct,
            "tothist_g_correct": tothist_g_correct,
...
def AmpSweep_ge_ReadoutSNR_withave_monoThermalReadout_FTmethod(
    ave=5, photonnum=np.arange(0.2, 8 + 0.3, 0.5), ifsave=1
):
...
def AmpSweep_ge_ReadoutSNR_withave_coherentReadout(
    ave=5, amp=np.arange(0, 1.41, 0.05), ifsave=1, ifprint=0, numrec=3000
):
    ...
    for m in np.arange(ave):
        for k in np.arange(np.size(amp)):
            ...
            check_readout_multiplestate(
                measure="coherent", num_steps=5, measTime=2000, c_amp=amp[k]
            )

            _, voltage, hist_g, hist_e, angle_c = (
                xr.compare_ge_readout_simultaneously_noSequence(
                    num_steps=5,
                    numrec=numrec,
                    ifplot=0,
...
def AmpSweep_ge_ReadoutSNR_withave_coherentReadout_FTmethod(
    ave=5,
    amp=np.arange(0, 1.41, 0.05),
    ifsave=1,
    ifprint=0,
    numrec=4000,
    weight="fixed",
...
def ac_Stark_shift(
    ...
    if readout == "coherent":
        ...
        main_pulse = Pulse(
            start=file_length - 6050, duration=2000, amplitude=thermal_amp
        )
        ringupdown_seq.add_sweep(channel=3, sweep_name="none", initial_pulse=main_pulse)

    acStark_ge = Pulse(
        start=file_length - 6050 + start,
        duration=width,
        amplitude=acStark_amp,
        ssm_freq=ssm_ge,
        phase=0,
    )  # pulse is also a class p is an instance
    ringupdown_seq.add_sweep(
...
def time_sweep_acStark(
    singleAve=10,
    amp=0.4,
    acStark_amp=0.05,
    width=200,
    start=np.arange(-400, 2801, 200),
    freqrange=[-0.07, 0.03],
    ifprint=0,
    ifsave=1,
):
    ...
    for k in np.arange(np.size(start)):
        ...
        sweepparam, prob = readout_python(
            num_patterns=num_steps,
            num_records=200,
            sweepparam=np.linspace(freqrange[0], freqrange[1], num_steps),
            ave=singleAve,
            ifprint=0,
            ifsave=0,
            ifplot=0,
    ...
    if ifsave:
        data = {"sweepparam": sweepparam, "totprob": totprob, "start": start}
        root = Tk()
        root.wm_attributes("-topmost", 1)
        savename = asksaveasfile(
            initialdir="C:\\Data\\2020\\201021_thermal_readout\\03_single_photon",
...
def time_sweep_acStark_thermal(
    singleAve=10,
    acStark_amp=0.05,
    width=200,
    start=np.arange(-400, 3000, 200),
    freqrange=[-0.07, 0.03],
    ifprint=0,
    ifsave=1,
):
...
def timeAndPower_sweep_acStark_thermal(
    singleAve=10,
    acStark_amp=0.05,
    width=200,
    start=np.arange(-400, 3000, 200),
    freqrange=[-0.07, 0.03],
    att=np.arange(40, 5, -2),
    ifprint=0,
    ifsave=1,
):
    num_steps = 101
    ...
    for k in np.arange(np.size(start)):
        ...
        for m in np.arange(np.size(att)):
            ...
            plt.ylabel("$P_e$")
            plt.title(
                "# {} in {}: {} ns Start {} dB att".format(
                    m + 1 + k * np.size(att),
                    np.size(start) * np.size(att),
                    start[k],
                    att[m],
                )
            )
...
def ampAndTime_sweep_acStark(
    singleAve=10,
    amp=np.arange(0, 1.41, 0.2),
    acStark_amp=0.03,
    width=300,
    start=np.arange(-600, 2701, 300),
    freqrange=[-0.07, 0.03],
    ifprint=1,
    ifsave=1,
):
    ...
    for m in np.arange(np.size(amp)):
        for k in np.arange(np.size(start)):
            ...
            plt.ylabel("$P_e$")
            plt.title(
                "# {} in {}: {} ns Start {} amp".format(
                    k + 1 + m * np.size(start),
                    np.size(start) * np.size(amp),
                    start[k],
                    amp[m],
                )
            )
...
def check_readout_multiplestate(
    measure="highpower",
    num_steps=5,
    measTime=2000,
    c_amp=1,
    ...
    if measure == "highpower":
        main_pulse = Pulse(
            start=file_length - 5000, duration=measTime, amplitude=readout_amp
        )
        ringupdown_seq.add_sweep(
            channel=1, marker=2, sweep_name="none", initial_pulse=main_pulse
        )
        ...
        main_pulse = Pulse(
            start=file_length - 5000, duration=measTime, amplitude=noise_amp
        )
        ringupdown_seq.add_sweep(channel=3, sweep_name="none", initial_pulse=main_pulse)
        ...
        main_pulse = Pulse(
            start=file_length - 5000, duration=measTime, amplitude=c_amp * np.cos(phase)
        )
        ringupdown_seq.add_sweep(channel=3, sweep_name="none", initial_pulse=main_pulse)
        main_pulse = Pulse(
            start=file_length - 5000,
            duration=measTime,
            amplitude=c_amp * np.sin(-phase),
        )
    ...
    if ifplotsequence:
        ...
        plt.figure()
        plt.imshow(
            channel[:, file_length - 5000 - 300 : file_length - 5000 + 50],
            aspect="auto",
        )
        plt.show()
...
def check_readout_multiplestate_differentPhase(
    measTime=2000, c_amp=1, phase=np.arange(0, 2 * np.pi, np.pi / 4), ifplotsequence=1
):  # this is pulsed readout to ring up and ring down cavity dfor e state
    # ===========================
    # g state
    num_steps = np.size(phase)
    ...
    for k in np.arange(num_steps):
        ...
        ringupdown_seq.channel_list[2][0][k, pulseStart : pulseStart + pulseLength] = (
            c_amp * np.cos(phase[k])
        )
        ringupdown_seq.channel_list[3][0][k, pulseStart : pulseStart + pulseLength] = (
            c_amp * np.sin(-phase[k])
        )
...
def check_readout_multiplestate_ef(
    measure=1, num_steps=5, measTime=2000
):  # this is pulsed readout to ring up and ring down cavity dfor e state
    # ===========================
    # g state
    ...
    pi_ge = Pulse(
        start=file_length - 5050 - pi_ge_time,
        duration=-pi_ge_time,
        amplitude=ge_amp,
        ssm_freq=ssm_ge,
        phase=0,
    ...
    pi_ge = Pulse(
        start=file_length - 5050 - pi_ef_time,
        duration=-pi_ge_time,
        amplitude=ge_amp,
        ssm_freq=ssm_ge,
        phase=0,
...
def readout_python_withInfo(num_records=200, ifplot=0, ifsave=0, ave=1, ifprint=1):
    ...
    for k in np.arange(ave):
        daq_params, rec_all_raw_ave = dp.run_daq_rawdata(
            num_patterns=num_patterns, num_records_per_pattern=num_records
        )

        _, currentprob = xr.process_readout_pattern(
            rec_all_raw_ave, daq_params, ifplot=0
        )
...
def readout_python(
    sweepparam=np.linspace(0, 50, 101),
    num_patterns=101,
    num_records=200,
    ifplot=0,
    ifsave=0,
...
def program_stop_button():
    ...
    def scanning():
        if running:  # Only do this if the Stop button has not been clicked
            print("start scan")
            xr.compare_ge_readout_simultaneously_noSequence(
                num_steps=5, numrec=1000, ifplotSNR=0, ifplotamp=0
            )
