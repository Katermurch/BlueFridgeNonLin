...
def sine_decay_with_decay_time(time, freq_Hz, T, amplitude, phase_deg, offset):
 #    phase_deg = 90. # fix the phase
     return amplitude*np.exp(-(1/T)*time)*np.sin(time*2*np.pi*freq_Hz + 
                            np.pi*phase_deg/180) + offset                        
def sine_decay_with_modulation(time, freq_Hz1,freq_Hz2, gamma, amplitude1,amplitude2, phase_deg1, phase_deg2, offset):
#    phase_deg = 90. # fix the phase
    return amplitude1*np.exp(-gamma*time)*np.sin(time*2*np.pi*freq_Hz1 + 
                           np.pi*phase_deg1/180) + amplitude2*np.sin(time*2*np.pi*freq_Hz2 + 
                           np.pi*phase_deg2/180) + offset
