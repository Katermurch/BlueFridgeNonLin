...
class Sequence:
    ...
    def insert_waveform(self, channel_num, pulse, step_index):
        ...
        step_waveform = self.channel_list[channel_num-1][0][step_index] # 0 for waveform
    ...
    def _gen_sweep(self, sweep_name, start, stop, dest_wave, initial_pulse=Pulse(amplitude=0, duration=0,start=0)):
        ...
        if sweep_name == 'none':
            ...
            for step_index, param_val in enumerate(np.linspace(start[0],stop[0],self.num_steps)):
                updated_pulse.duration= initial_pulse.duration + int(param_val)
...
#            print([start, stop, self.num_steps])
            for step_index, param_val in enumerate(np.linspace(start,stop,self.num_steps)):
                updated_pulse.ssm_freq= initial_pulse.ssm_freq + param_val
                file_length = self.sequence_length
                updated_pulse.phase= initial_pulse.phase+ -file_length*param_val*360
                gen_pulse( dest_wave = dest_wave[step_index], pulse=updated_pulse)
    ...
    def write_sequence_to_disk(self, base_name='foo', file_path=os.getcwd(), use_range_01=False,num_offset=0, write_binary=False): 
        ''' 
    ...
    def load_sequence_from_disk(self, instr_addr, base_name='foo', file_path=os.getcwd(), num_offset=0, ch_amp=[1,1,1,1]):
        ''' 
...
#        offset = [-0.021, -0.094, -0.006, -0.021] # for KZ047R111 qubit
        ...
        wx_programs.set_marker_level(which_channel='ch1ch2', which_marker=1, marker_voltage_level = 0.5)
        wx_programs.set_marker_level(which_channel='ch3ch4', which_marker=2, marker_voltage_level = 0.5)
