...
def fit_exp_decay_with_decay_time(x_vals, y_vals, guess_vals=None):
    ...
    #
    popt, pcov = curve_fit(fitfun.exp_decay_with_decay_time, x_vals, y_vals, p0=guess_vals,maxfev=3000)
    perr = np.sqrt(np.diag(pcov))
...
# def fit_fast_fourier_transform(x_vals, y_vals,num_steps):
#     #borrowed from xingrui's code
#     spec = fft(y_vals)
#     spec_freq = fftfreq(len(x_vals), num_steps)
#     mask = spec_freq > np.max(spec_freq)
#     # mask = spec_freq > 0
#     spec_freq = spec_freq[mask]
#     spec = spec[mask]

   
#     plt.plot(spec_freq, np.abs(spec) ** 2)
#     plt.show()
    
#     i_peak = np.argmax(np.real(spec) ** 2 + np.imag(spec) ** 2)
#     f_peak = spec_freq[i_peak]
#     s_real = np.real(spec[i_peak])
#     s_imag = np.imag(spec[i_peak])

#     omega = 2 * np.pi * f_peak
#     phi = np.arctan2(s_imag, s_real)

#     A = np.mean(I_avg_1)
#     B = 4 * np.sqrt(s_real ** 2 + s_imag ** 2) / len(I_avg_1)
#     Q1_T2 = t_clk_max * 4
...
def fit_sine_decay_with_decay_time(x_vals,y_vals, guess_vals=None):
    if guess_vals is None:
        ...
        guess_vals = [guess_freq_Hz, guess_T, guess_amplitude, guess_phase_deg, guess_offset]
    ...
    y_vals_fit = np.full(len(x_vals), np.nan)
    try:
        popt, pcov = curve_fit(fitfun.sine_decay_with_decay_time, x_vals, y_vals, p0=guess_vals)
...
def fit_sine_decay_with_modulation(x_vals, y_vals, guess_vals=None):
    if guess_vals is None:
        ...
        guess_vals = [guess_freq_Hz1,guess_freq_Hz2, guess_gamma, guess_amplitude1,guess_amplitude2, guess_phase_deg1,guess_phase_deg2, guess_offset]
    ...
    y_vals_fit = np.full(len(x_vals), np.nan)
    try:
        popt, pcov = curve_fit(fitfun.sine_decay_with_modulation, x_vals, y_vals, p0=guess_vals)
    ...
    print("\n")
    var_str = ["  freq1","  freq2", " gamma", "   amp1","   amp2", " phase1"," phase2", "offset"]
    for idx, v in enumerate(var_str):
        print(v+": {} +/- {}".format(popt[idx], perr[idx]))
...
def fit_sine_fix_freq(x_vals, y_vals, guess_vals=None,fixed_freq=.1 ):
    popt, pcov = curve_fit(lambda timep, amplitude2, phase_rad2, offset2: fitfun.sine(timep,fixed_freq, amplitude2, phase_rad2, offset2), x_vals, y_vals, p0=guess_vals)
    perr = np.sqrt(np.diag(pcov))
